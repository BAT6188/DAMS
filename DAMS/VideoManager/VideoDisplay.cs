...
namespace DAMS.VideoManager
{
    class VideoDisplay
    {
        //FFmpeg配置信息
        public string ffmpegpath = "ffmpeg.exe";//FFmpeg的服务器路径
        private string imgsize = "400*300";     //视频截图大小
        private string videosize = "640*480"; //视频大小
        ...
        public string Convert(string sourceFile, string destPath, string uniquename, VideoType videotype, bool createImage, bool getDuration)
        {
            ...
            if (Path.GetExtension(sourceFile).ToLower() != GetDiscription(videotype).ToLower())
            {
                 ...
                 * 最后的路径为目标文件
                 */
                FilestartInfo.Arguments = " -i " + sourceFile + " -ab 128000 -vcodec mpeg4 -b 1200000 -mbd 2 -s " + videosize + " " + destFile;
            ...
            if (createImage)
            {
                ...
                ImgstartInfo.RedirectStandardInput = true;
        ...
        /// <param name="sourceFile">要转换文件绝对路径</param>
        /// <param name="destPath">转换结果存储的相对路径</param>
        /// <param name="uniquename">缩略图名称</param>
        /// <returns>
        /// 执行成功返回空，否则返回错误信息
        /// </returns>
        public string ConvertImageForAll(string sourceFile, string destPath, string uniquename)
        {
            //取得ffmpeg.exe的物理路径
            string ffmpeg = ffmpegpath;
            if (!File.Exists(ffmpeg))
            ...
            if (!File.Exists(sourceFile))
            {
                ...
                //截大图
                string imgpath = destPath + uniquename + ".jpg";
        ...
        /// 执行成功返回空，否则返回错误信息
        /// </returns>
        public string Compress(string sourceFile, string destPath, string uniquename, VideoType videotype)
        {
            //取得ffmpeg.exe的物理路径
            string ffmpeg = ffmpegpath;
            if (!File.Exists(ffmpeg))
            ...
            //string uniquename = FileHelper.GetUniqueFileName();
            string filename = uniquename + ".MP4";
            string destFile = destPath + filename;
                System.Diagnostics.ProcessStartInfo FilestartInfo = new System.Diagnostics.ProcessStartInfo(ffmpeg);
                FilestartInfo.WindowStyle = System.Diagnostics.ProcessWindowStyle.Hidden;
                FilestartInfo.CreateNoWindow = true;
                ...
                {
                    //转换
                    System.Diagnostics.Process.Start(FilestartInfo).WaitForExit();
        ...
        private string ConvertImage(string sourceFile, string imgpath, string imgsize, System.Diagnostics.ProcessStartInfo ImgstartInfo)
        {
            ...
            {
                MessageBox.Show(ex.Message, MainControl.MainConst.MESSAGE_BOX_TITLE);
                return "";
            }
        }
        ...
        public string GetVideoDuration(string ffmpegfile, string sourceFile)
        {
            using (System.Diagnostics.Process ffmpeg = new System.Diagnostics.Process())
            {
                ...
                ffmpeg.StartInfo.RedirectStandardInput = true;//可能接受来自调用程序的输入信息 
                ffmpeg.StartInfo.RedirectStandardOutput = true;//由调用程序获取输出信息 
